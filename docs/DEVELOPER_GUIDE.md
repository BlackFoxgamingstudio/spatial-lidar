# Developer Guide: Sovereign Spatial LiDAR
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/spatial-lidar.git
cd spatial-lidar
...
```bash
export SBB_LIDAR_DEVICE=your_value_here
export SBB_POINT_CLOUD_DENSITY=your_value_here
export SBB_SEGMENTATION_MODEL=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-spatial-lidar)
...
```bash
# Build
docker build -t sovereign-spatial-lidar:dev .

# Run with env file
docker run --env-file .env -p 8803:8803 sovereign-spatial-lidar:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-spatial-lidar
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8803`
...
requests.post(N8N_EVENT_BUS_URL, json={
    "source": "sovereign-spatial-lidar",
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8803` in use | `lsof -i :8803` | Kill PID or change `SERVICE_PORT` |
