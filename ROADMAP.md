# Engineering Roadmap & Implementation Status — Sovereign Spatial LiDAR

**Package ID**: `PKG-000`  
**Domain**: ** Spatial Computing & Apple LiDAR  
**Microservice Port**: `http://127.0.0.1:8803`  
...
| **Networking & API** | REST Microservice (`PORT 8803`) | ✅ Complete | OpenAPI 3.1 spec, Swagger UI at `/docs` |
...
### Component 1: `PointCloudCapture`
- **Role**: Handles all pointcloudcapture operations. Exposes async methods callable from the core dispatcher.
- **Current Status**: ⏳ Queued for v1.1.0 Implementation
- **Integration**: Exposed via `POST /api/v1/execute` with action `pointcloudcapture`
- **Verification Strategy**: Dedicated `unittest.TestCase` validating deterministic output and idempotency.

### Component 2: `ObjectSegmentor`
- **Role**: Handles all objectsegmentor operations. Exposes async methods callable from the core dispatcher.
- **Current Status**: ⏳ Queued for v1.1.0 Implementation
- **Integration**: Exposed via `POST /api/v1/execute` with action `objectsegmentor`
- **Verification Strategy**: Dedicated `unittest.TestCase` validating deterministic output and idempotency.

### Component 3: `DimensionalMeasurer`
- **Role**: Handles all dimensionalmeasurer operations. Exposes async methods callable from the core dispatcher.
- **Current Status**: ⏳ Queued for v1.1.0 Implementation
- **Integration**: Exposed via `POST /api/v1/execute` with action `dimensionalmeasurer`
- **Verification Strategy**: Dedicated `unittest.TestCase` validating deterministic output and idempotency.

### Component 4: `MeshReconstructor`
- **Role**: Handles all meshreconstructor operations. Exposes async methods callable from the core dispatcher.
- **Current Status**: ⏳ Queued for v1.1.0 Implementation
- **Integration**: Exposed via `POST /api/v1/execute` with action `meshreconstructor`
- **Verification Strategy**: Dedicated `unittest.TestCase` validating deterministic output and idempotency.

### Component 5: `ModelExporter`
- **Role**: Handles all modelexporter operations. Exposes async methods callable from the core dispatcher.
- **Current Status**: ⏳ Queued for v1.1.0 Implementation
- **Integration**: Exposed via `POST /api/v1/execute` with action `modelexporter`
- **Verification Strategy**: Dedicated `unittest.TestCase` validating deterministic output and idempotency.
...
1. Create discrete module file: `src/spatial_lidar_<component>.py`
